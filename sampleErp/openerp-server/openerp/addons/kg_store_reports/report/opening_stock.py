...
class opening_stock(report_sxw.rml_parse):
	...
	def __init__(self, cr, uid, name, context=None):
		if context is None:
			context = {}
		super(opening_stock, self).__init__(cr, uid, name, context=context)
		self.query = ""
		self.period_sql = ""
	...
	def get_data(self,form):
		...
		self.cr.execute('''		
		  ...
		  SELECT distinct on (sm.product_id)		  			
			...
			order by sm.product_id''',('in','done',form['date_from'],form['date_to']))		   
...
report_sxw.report_sxw('report.opening.stock', 'stock.picking', 
			'addons/kg_grn/report/opening_stock.rml', 
			parser=opening_stock, header = False)
