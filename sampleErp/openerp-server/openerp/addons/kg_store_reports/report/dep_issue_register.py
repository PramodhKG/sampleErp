	...
	def get_data(self,form):
		...
		if form['dep_id']:
			for ids1 in form['dep_id']:
				partner.append("sp.dep_name = %s"%(ids1))
		...
		self.cr.execute('''
			  ...
			  where sp.type=%s and sp.state=%s and sp.date >=%s and sp.date <=%s'''+ where_sql + partner + product + '''
		...
		for pos1, item1 in enumerate(data):
			...
			for pos2, item2 in enumerate(data):
				if not pos1 == pos2:
					if item1['sp_id'] == item2['sp_id'] and item1['date'] == item2['date']:
						...
						if len(move_id) != 1:
							...
							for move in move_id:
								...
								tot += move_rec.po_to_stock_qty * move_rec.stock_rate
							item1['iss_total'] = tot
						else:
							item1['iss_total'] = 0.0						
						if count == 0:
