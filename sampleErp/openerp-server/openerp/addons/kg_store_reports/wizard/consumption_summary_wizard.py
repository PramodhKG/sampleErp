...
class consumption_summary_wizard(osv.osv_memory):
		
	_name = 'consumption.summary.wizard'
	_columns = {
		
		'filter': fields.selection([('filter_date', 'Date')], "Filter by", required=True),
		'date_from': fields.date("Start Date"),
		'date_to': fields.date("End Date"),
		'dep_id': fields.many2many('kg.depmaster', 'cons_summary_dep', 'dep_id','move_id','Department Name'),
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			return {'type': 'ir.actions.report.xml', 'report_name': 'consumption.summary', 'datas': data,  'name': 'Consumption Summary'}	
		

consumption_summary_wizard()
