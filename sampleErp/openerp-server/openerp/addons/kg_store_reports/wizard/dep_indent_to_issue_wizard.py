...
class dep_indent_to_issue_wizard(osv.osv_memory):
		
	_name = 'dep.indent.to.issue.wizard'
	_columns = {
		
		'dep_id':fields.many2many('kg.depmaster','dep_indent_issue','indent_id','dep_id','Department Name'),
		'product_id': fields.many2many('product.product','dep_indent_issue_pro','indent_id','pro_id','Product Name'),
	...
	_constraints = [
		(_date_validation_check, 'You must select a correct Start Date and End Date !!', ['Valid_date']),
	  ]
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			return {'type': 'ir.actions.report.xml', 'report_name': 'dep.indent.to.issue', 'datas': data}	
		

dep_indent_to_issue_wizard()
