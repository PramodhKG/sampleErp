...
class issue_summary_from_mainst(osv.osv_memory):
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			dep_name = data['form']['dep_id'][-1]
			data['form']['dep'] = dep_name
			data['form']['date_from_ind'] = self.date_indian_format(date_from)			
			data['form']['date_to_ind'] = self.date_indian_format(date_to)
			return {'type': 'ir.actions.report.xml', 'report_name': 'issue.summary.from.mainst', 'datas': data,  'name': 'Department Issue Summary'}	
		

issue_summary_from_mainst()
