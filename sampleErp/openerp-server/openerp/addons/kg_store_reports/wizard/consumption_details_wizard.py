...
class consumption_details_wizard(osv.osv_memory):
		
	_name = 'consumption.details.wizard'
	_columns = {
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			if data['form']['dep_id']:
				dept = data['form']['dep_id']
				dep_rec=self.pool.get('kg.depmaster').browse(cr,uid,dept[0])
				depart=dep_rec.dep_name
				data['form']['department']=depart
				
			return {'type': 'ir.actions.report.xml', 'report_name': 'consumption.details.report', 'datas': data,  'name': 'Consumption Details'}	
		

consumption_details_wizard()
