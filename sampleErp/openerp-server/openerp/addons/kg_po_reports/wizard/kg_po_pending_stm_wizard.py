...
class kg_po_pending_stm_wizard(osv.osv_memory):
		
	_name = 'kg.po.pending.stm.wizard'
	_columns = {
		
		'product_id':fields.many2many('product.product','kg_po_pending_stm_pro','order_id','product_id','Product Name'),
		'supplier':fields.many2many('res.partner','kg_po_pending_stm_sup','order_id','supplier_id','Supplier'),
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			return {'type': 'ir.actions.report.xml', 'report_name': 'po.pending.stm', 'datas': data,  'name': 'PO Pending Statement'}
	
	
		

kg_po_pending_stm_wizard()
