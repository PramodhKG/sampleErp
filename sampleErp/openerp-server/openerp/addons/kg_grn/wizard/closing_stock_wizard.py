...
class closing_stock_wizard(osv.osv_memory):
	...
	_columns = {
		...
		'location_dest_id': fields.many2one('stock.location', 'Stores', required=True, domain="[('location_type','=', 'sub')]"),
		'major_name':fields.many2one('product.category', 'Product Category'),
		'product':fields.many2many('product.product','close_stock_product','stock_product_id','close_stock_id','Product'),
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			return {'type': 'ir.actions.report.xml', 'report_name': 'closing.stock.wizard', 'datas': data}	
