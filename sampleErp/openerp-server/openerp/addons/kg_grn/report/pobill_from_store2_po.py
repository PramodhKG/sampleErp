...
class pobill_from_store2_po(report_sxw.rml_parse):
	
	_name = 'report.pobill.from.store'
	_inherit='stock.picking'   

	def __init__(self, cr, uid, name, context=None):
		if context is None:
			context = {}
		super(pobill_from_store2_po, self).__init__(cr, uid, name, context=context)
		self.query = ""
		self.period_sql = ""
	...
	def get_data(self,form):
	   ...
	   Write sql or orm queries to get detail as a list of dict
		...
		self.cr.execute('''
			  ...
			  inv.inv_confirm_date >=%s and inv.inv_confirm_date <=%s'''+ where_sql + partner +'''
			   order by inv.name''',('in_invoice','draft',form['date_from'],form['date_to']))
...
report_sxw.report_sxw('report.pobill.from.store', 'stock.picking', 
			'addons/kg_grn/report/pobill_from_store2_po.rml', 
			parser=pobill_from_store2_po, header = False)
