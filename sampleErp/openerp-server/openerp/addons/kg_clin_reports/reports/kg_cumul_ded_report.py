...
class kg_cumul_ded_report(report_sxw.rml_parse):
	...
	def get_data(self,form):
		...
		print "month ,year ",form['month'],form['year']
		self.cr.execute('''
			 ...
			 select 
			  ...
			  where extract(month from ded.date) >=%s and extract(year from ded.date)<=%s and ded.state = %s'''+ employee + '''
			  order by ded.employee_id''',(form['month'],form['year'],'approve'))
...
report_sxw.report_sxw('report.kg.cumul.ded.wizard', 'hr.payslip', 
			'addons/kg_clin_reports/reports/kg_cumul_ded_report.rml', 
