...
class kg_allowance_report(report_sxw.rml_parse):
	...
	def get_data(self,form):
		...
		self.cr.execute('''
			...
			where entry.state!=%s and extract (month from entry.start_date) >=%s and extract(year from entry.end_date) <=%s '''+ where_sql + 
				pay_type + ''' order by emp.id''',('cancel', form['month'],form['year']))
...
report_sxw.report_sxw('report.kg.allowance.report', 'hr.payslip', 
			'addons/kg_clin_reports/reports/kg_allowance_report.rml', 
