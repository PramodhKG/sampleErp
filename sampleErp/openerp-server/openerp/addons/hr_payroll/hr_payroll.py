...
class hr_payslip(osv.osv):
	...
	def _get_lines_salary_rule_category(self, cr, uid, ids, field_names, arg=None, context=None):
		print "_get_lines_salary_rule_category ()))))))))))))))()))()()"
		result = {}
		if not ids: return result
		for id in ids:
			result.setdefault(id, [])
		cr.execute('''SELECT pl.slip_id, pl.id FROM hr_payslip_line AS pl \
					LEFT JOIN hr_salary_rule AS sh on (pl.category_id = sh.category_id) \
...
#			old_slipline_ids
			...
			if payslip.contract_id:
				...
				print "contract_ids---------------,", contract_ids, "payslip.contract_id=======", payslip.contract_id
			else:
	...
	def get_payslip_lines(self, cr, uid, contract_ids, payslip_id, context):
		...
		def _sum_salary_rule_category(localdict, category, amount):
			...
			if category.parent_id:
				...
				print "222222-----localdict-----------", localdict
			localdict['categories'].dict[category.code] = category.code in localdict['categories'].dict and localdict['categories'].dict[category.code] + amount or amount
			print "33333333=====localdict---------", localdict
			return localdict
		...
		class Payslips(BrowsableObject):
			print "class Payslips(BrowsableObject) ---------- called"
			"""a class that will be used into the python code, mainly for usability purposes"""
...
class hr_payslip_worked_days(osv.osv):
	...
	_columns = {
		...
		'code': fields.char('Code', size=52, help="The code that can be used in the salary rules"),
...
class hr_salary_rule(osv.osv):
	...
	_columns = {
		...
		'category_id':fields.selection([('BASIC','Basic'),('GROSS','Gross'),('ALW','Allowance'),('DED','Deduction'),
										('NET','Net'),('COMP','Company Contribution')], 'Category', required=True),
