...
class kg_nonpf_paymuster(osv.osv_memory):
		
	_name = 'kg.nonpf.paymuster'
	
	_columns = {
		
		'dep_id':fields.many2many('hr.department','kg_paymuster2_dep','wiz_id','dep_id','Department Name'),
	...
	_defaults = {
		...
		'pay_sch': '7th',
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.nonpf.paymuster', context=c),
	}
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			return {'type': 'ir.actions.report.xml', 'report_name': 'kg.nonpf.paymuster', 'datas': data}
