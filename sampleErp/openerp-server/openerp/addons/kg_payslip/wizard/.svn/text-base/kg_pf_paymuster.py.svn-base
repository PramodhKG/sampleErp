...
class kg_pf_paymuster(osv.osv_memory):
		
	_name = 'kg.pf.paymuster'
	
	_columns = {
		
		'dep_id':fields.many2many('hr.department','kg_paymuster_dep','wiz_id','dep_id','Department Name'),
	...
	_defaults = {
		...
		'pay_sch': '7th',
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.pf.paymuster', context=c),
	}
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			return {'type': 'ir.actions.report.xml', 'report_name': 'kg.pf.paymuster', 'datas': data}
