...
class kg_netpayment(osv.osv_memory):
		
	_name = 'kg.netpayment'
	
	_columns = {
		
		'dep_id':fields.many2many('hr.department','kg_netpay_dep','wiz_id','dep_id','Department Name'),
		'pay_mode':fields.selection([('cheque','THROUGH CHEQUE'),('bank','THROUGH BANK'),
							('cash','THROUGH CASH')], 'Payment Mode', required=True),
	...
	_defaults = {
		...
		'pay_sch': '7th',
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.netpayment', context=c),
	}
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			if data['form']['pay_mode']:
				pay_mode = data['form']['pay_mode']				
				if pay_mode == 'bank':
					data['form']['mode'] = 'BANK - '
				elif pay_mode == 'cash':
					data['form']['mode'] = 'CASH - '
				else:
					data['form']['mode'] = 'CHEQUE - '
			else:
				data['form']['mode'] = False
				
			return {'type': 'ir.actions.report.xml', 'report_name': 'kg.netpayment', 'datas': data}
