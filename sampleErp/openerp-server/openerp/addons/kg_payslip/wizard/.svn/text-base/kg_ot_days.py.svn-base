...
class kg_ot_days(osv.osv_memory):
		
	_name = 'kg.ot.days'
	
	_columns = {
		
		'dep_id':fields.many2many('hr.department','kg_otdays_dep','wiz_id','dep_id','Department Name'),
	...
	_defaults = {
		...
		'pay_sch': '7th',
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.ot.days', context=c),
	}
