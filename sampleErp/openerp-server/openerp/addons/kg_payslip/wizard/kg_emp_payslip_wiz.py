...
class kg_emp_payslip_wiz(osv.osv_memory):
		
	_name = 'kg.emp.payslip.wiz'
	
	_columns = {
		
		'dep_id':fields.many2many('hr.department','kg_dept_payslip','payslip_id','dept_pay_id','Department Name'),
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.emp.payslip.wiz', context=c),
	}
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			print "employee>>>>>>>>>>>>>>>>>>>>",data['form']['employee']
			...
			return {'type': 'ir.actions.report.xml', 'report_name': 'kg.emp.payslip.rpt', 'datas': data}
