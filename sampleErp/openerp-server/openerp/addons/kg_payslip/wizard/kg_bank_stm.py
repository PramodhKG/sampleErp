...
class kg_bank_stm(osv.osv_memory):
		
	_name = 'kg.bank.stm'
	
	_columns = {
		
		'dep_id':fields.many2many('hr.department','kg_emp_bank_dep','dep_id','slip_id','Department Name'),
	...
	_defaults = {
		...
		'pay_sch': '7th',
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.bank.stm', context=c),
	}
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			if data['form']['bank_id']:
				bank_id = data['form']['bank_id'][0]
				bank_rec = self.pool.get('res.bank').browse(cr,uid,bank_id)
				data['form']['bank_name'] = bank_rec.name
			else:
				data['form']['bank_name'] = False
			...
			return {'type': 'ir.actions.report.xml', 'report_name': 'kg.emp.bank.stm', 'datas': data}
