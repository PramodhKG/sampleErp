...
class kg_pay_abstract(osv.osv_memory):
		
	_name = 'kg.pay.abstract'
	
	_columns = {
		
		'dep_id':fields.many2many('hr.department','kg_pay_abstract_dep','wiz_id','dep_id','Department Name'),
	...
	_defaults = {
		...
		'pay_sch': '7th',
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.pay.abstract', context=c),
	}
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			return {'type': 'ir.actions.report.xml', 'report_name': 'kg.pay.abstract', 'datas': data}
