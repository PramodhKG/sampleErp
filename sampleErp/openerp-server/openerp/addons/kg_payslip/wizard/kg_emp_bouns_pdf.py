...
class kg_emp_bouns_pdf(osv.osv_memory):
	...
	_columns = {
		...
		'dep_id':fields.many2many('hr.department','kg_bouns_dep','wiz_id','dep_id','Department Name'),
	...
	_defaults = {
		...
		'company_id': lambda self,cr,uid,c: self.pool.get('res.company')._company_default_get(cr, uid, 'kg.emp.bouns.pdf', context=c),
	}
	...
	def _print_report(self, cr, uid, ids, data, context=None):
		...
		if data['form']:
			...
			data['form']['end'] = date_to.strftime('%B-%Y')
			
			company_id = data['form']['company_id'][0]
			com_rec = self.pool.get('res.company').browse(cr,uid, company_id)			
			data['form']['company'] = com_rec.name
			return {'type': 'ir.actions.report.xml', 'report_name': 'kg.emp.bouns.pdf', 'datas': data}
