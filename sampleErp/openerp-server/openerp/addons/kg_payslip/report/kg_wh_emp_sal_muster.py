...
class kg_wh_emp_sal_muster(report_sxw.rml_parse):
	
	_name = 'kg.wh.emp.sal.muster'
	_inherit='hr.payslip'   

	def __init__(self, cr, uid, name, context=None):
		if context is None:
			context = {}
		super(kg_wh_emp_sal_muster, self).__init__(cr, uid, name, context=context)
		self.query = ""
		self.period_sql = ""
	...
	def get_data(self,form):
		...
		if form['employee_id']:
			for ids3 in form['employee_id']:
				employee.append("slip.employee_id = %s"%(ids3))
		...
		self.cr.execute('''
			...
			left JOIN kg_monthly_attendance att ON (att.id=slip.att_id)		 			  
			  ...
			  where slip.state=%s and slip.date_from >=%s and slip.date_to <=%s'''+ where_sql + employee +'''
		...
		for val in data:			
			...
			val['tot_gross']=val['basic_amt'] + val['ot_amt'] + val['alw_amt'] + val['oear_amt'] + val['epf_amt']
...
report_sxw.report_sxw('report.kg.wh.emp.sal.muster', 'hr.payslip', 
			'addons/kg_payslip/report/kg_wh_emp_sal_muster.rml', 
			parser=kg_wh_emp_sal_muster, header = False)
