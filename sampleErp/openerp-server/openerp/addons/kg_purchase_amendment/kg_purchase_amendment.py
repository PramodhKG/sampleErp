...
class kg_purchase_amendment(osv.osv):	
	...
	_columns = {
		...
		'po_id':fields.many2one('purchase.order','PO.NO', required=True,
			domain="[('state','=','approved') ,'&',('order_line.line_state','!=','cancel'),'&',('order_line.line_bill','=', False)]",
	...
	def confirm_amend(self,cr,uid,ids,context={}):
		...
		for amend_line in amend_obj.amendment_line:
			...
			move_id = move_obj.search(cr, uid, [('purchase_line_id','=',po_line_id)])
			if move_id:
				move_rec = move_obj.browse(cr, uid, move_id[0])
			
			if po_rec.picking_ids:
				print "po_rec.picking_ids.....................",po_rec.picking_ids
				for pick in po_rec.picking_ids:
					...
					if pick.state != 'inv':					
						...
						grn_price = amend_line.price_unit_amend
						amend_tax = amend_line.taxes_id_amend or False
						print "amend_tax...........GRNNNNNN...............>>>>>>>>>>",amend_tax
						grn_price = round(grn_price, 2)
						
						print "grn_price........GRNNNN.........",grn_price
						...
						print "amend_line.id:GRNNNNN::taxes_id_amend:::::::", amend_line.taxes_id_amend
						...
						cr.execute(""" delete from purchase_order_taxxes where order_id=%s """ %(move_rec.id))
						
						
						
						for i in range(len(val)):
							print "IIIIIIIIGRNNNNNIIIIIIIIIIIII", val[i]
							
							cr.execute(""" INSERT INTO purchase_order_taxxes (order_id,taxes_id) VALUES(%s,%s) """ %(move_rec.id,val[i]))
						...
						lot_ids = lot_obj.search(cr, uid, [('grn_move','=',move_rec.id)])
						if lot_ids:
